...
# csv_file = pd.read_csv('./Dados/consulta_cand_2024_CE.csv', sep=';', encoding='latin1')
...
fig = px.bar(contagem_partido, x='SG_PARTIDO', y='count', color='count', 
             color_continuous_scale='Viridis', title='Quantidade de Candidatas Femininas por Partido')
st.plotly_chart(fig)
...
fig = px.bar(contagem_partido, x='SG_PARTIDO', y='count', color='count', 
             color_continuous_scale='Viridis', title='Quantidade de Candidatos Masculinos por Partido')
st.plotly_chart(fig)
